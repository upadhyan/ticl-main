...
def sampleAssignCal(X: torch.Tensor, aMatrix: torch.Tensor, bVector: torch.Tensor) -> torch.Tensor:
    ...
    z = torch.ones(n, dtype=torch.long, device=X.device)  
...
def update_c(X: torch.Tensor, y: torch.Tensor, treeDepth: int, tree: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    device = X.device
    tree['a'] = tree['a'].to(device)
    tree['b'] = tree['b'].to(device)
    y = y.to(device)

    # Initialize c and z
    c = torch.full([int(Tleaf)], (miny + maxy) / 2, device=device)
    z = torch.ones(n, device=device, dtype=torch.long)
